...
date = datetime(2020, 1, 1)  # Получать данные будем с первой возможной даты и времени Алгопака
...
def save_candles_to_file(symbol='SBER', time_frame='M',
    ...
    file_name = f'{datapath}{symbol}_{tf}.txt'
    ...
    if file_exists:  # Если файл существует
        ...
        last_dt = date  # Получать данные будем с первой возможной даты и времени Алгопака
    ...
    while True:  # Будем получать данные пока не получим все
        iterator = ticker.candles(date=last_date, till_date=till_date, period=time_frame, limit=limit)  # История. Максимум, 50000 баров
...
def save_metric_to_file(symbol='SBER', metric='tradestats'):
    ...
    while True:  # Будем получать данные пока не получим все
        if metric == 'tradestats':  # Сделки
            iterator = ticker.tradestats(date=last_date, till_date=till_date, limit=limit)
        elif metric == 'orderstats':  # Заявки
            iterator = ticker.orderstats(date=last_date, till_date=till_date, limit=limit)
        elif metric == 'obstats':  # Стакан
            iterator = ticker.obstats(date=last_date, till_date=till_date, limit=limit)
        else:
...
if __name__ == '__main__':  # Точка входа при запуске этого скрипта
    ...
    time_frames = ('D', 60, 10, 1)  # ВременнЫе интервалы
    # symbols = ('SBER',)  # Тикер для тестов
    # metrics = ('tradestats',)  # Метрика для тестов
    # time_frames = ('Q', 'M', 'W', 'D')  # ВременнЫе интервалы для тестов

    # save_candles_to_files(symbols, time_frames)  # Сохраняем бары тикеров по временнЫм интервалам в файлы
    save_metrics_to_files(symbols, metrics)  # Сохраняем метрики тикеров в файлы
