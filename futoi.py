...
class MOEXPy:
    """Работа с информационно-статистическим сервером Московской Биржи"""
    def __init__(self, login, password):
        self.session = requests.Session()  # Сессия запроса
        self.session.get('https://passport.moex.com/authenticate', auth=(login, password))  # Basic-аутентификация на сайте moex.com
        self.headers = {'User-Agent': self.session.headers['User-Agent']}  # python-requests/x.xx.x
        self.cookies = {'MicexPassportCert': self.session.cookies['MicexPassportCert']}  # Сертификат аутентификации. Будет отправляться при последующих запросах
        self.url = 'https://iss.moex.com/iss'  # Адрес информационно-статистического сервера Московской Биржи (ИСС/ISS)

    def futoi(self, symbol, from_date=None, to_date=None, latest=0, data_format='json'):
    ...
    def futoi_to_dataframe(self, symbol, from_date=None, to_date=None, latest=0):
        ...
        while not from_date or from_date <= t:  # Пока нужно делать запрос
            json = self.futoi(symbol, from_date, t, latest).json()  # Полученный ответ переводим в формат JSON, чтобы импортировать в pandas DataFrame
